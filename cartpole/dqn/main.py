...
def cartpole(args):
    ...
    while not solved and episodes < n_games:
        ...
        episodes += 1
        scores.append(score)
        solved = min(scores[-100:]) >= 195.0

        print(f"episode: {episodes} - highest score: {max(scores)}    \r", end="")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--file', help="The output filename for the plots. No plot will be created if unspecified.", type=str, default=None)
    parser.add_argument('-ep', '--epsilon', help="The starting epsilon value for the playing agent.", type=float, default=1.0)
